...
if(is.null(params$input_datatable)){
  # test with local test with local files in the same dir,
   data_fun <- read.csv("function_data_20190904.csv", header = TRUE, sep = ",")
...
#meta_table <- read.delim("function_meta_20190904.txt", header = TRUE, check.names = FALSE, stringsAsFactors = FALSE)
...
data_fun_COG <- data_fun_COG[which(rowSums(data_fun_COG[,-1])>0),]
...
data_fun_NOG <- data_fun_NOG[which(rowSums(data_fun_NOG[,-1])>0),]
...
data_fun_KEGG <- data_fun_KEGG[which(rowSums(data_fun_KEGG[,-1])>0),]
...
`r if(ncol(data_fun_COG) > 1){ "# Heatmap of functional composition"} `
`r if(ncol(data_fun_COG) > 1){ "## Heatmap of COG composition"} `
...
if(is.null(meta_table)){
  d3heatmap::d3heatmap(data_fun_COG[,-1],show_grid = FALSE, color = "RdYlBu", scale = "row")
}else{
  library(pheatmap)  
  Group <- meta_table$Grouping
  meta <- as.data.frame(Group)
  rownames(meta) <- colnames(data_fun_COG[,-1])
  pheatmap(data_fun_COG[,-1], annotation_col = meta, scale = "row")
}


```

`r if(ncol(data_fun_NOG) > 1){ "## Heatmap of NOG composition"} `
...
if(is.null(meta_table)){
  d3heatmap::d3heatmap(data_fun_NOG[,-1],show_grid = FALSE, color = "RdYlBu", scale = "row")
}else{
  pheatmap(data_fun_NOG[,-1], annotation_col = meta, scale = "row")
}


```

`r if(ncol(data_fun_KEGG) > 1){ "## Heatmap of KEGG composition"} `
...
if(is.null(meta_table)){
  d3heatmap::d3heatmap(data_fun_KEGG[,-1],show_grid = FALSE, color = "RdYlBu", scale = "row")
}else{
  pheatmap(data_fun_KEGG[,-1], annotation_col = meta, scale = "row")
}
...
df_COG_log10 <- log10(data_fun_COG[,-1]+1)
...
if(is.null(meta_table)){
  ...
  PCA_result <- prcomp(t(df_COG_log10))
  loading <- as.data.frame(PCA_result$x)
  meta_3D_PCA <- cbind(rownames(loading), meta_table$Grouping)
...
df_NOG_log10 <- log10(data_fun_NOG[,-1]+1)
...
df_KEGG_log10 <- log10(data_fun_KEGG[,-1]+1)
