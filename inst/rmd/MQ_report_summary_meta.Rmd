...
params:  
  summary_file_tbl: !r 1 ### manditory note that the value is the real value, here supposed to be a read in data.table for one click function.
  meta_table: !r 1 ### optional this meta is optional, if provided, will do some statistics
---
...
# Note: The folling analysis with meta infor assumes that
# 1st columns as sample name, 2/3 column as grouping

if(length(params$meta_table) >1 ){ 
  meta_table <- params$meta_table 
}else{
  # meta_table <- read.delim("summary1_meta.txt", header = TRUE, check.names = FALSE, stringsAsFactors = FALSE) # test with meta file
  meta_table <- NULL # test without meta file
}
# meta_table_input <- read.delim("summary1_meta.txt", header = TRUE, check.names = FALSE, stringsAsFactors = FALSE)
...
`r if( !is.null(meta_table)){ c("* **Groups: **",unique(meta_table[,3]))} `
...
2. MS ID rate should also be well-reproduced across samples and groupings. 
  ...
  *  A decreasing MS ID treand along sample run time indicates the performance drop of the MS: your MS might need to be cleaned. If you did not randomize the sample run-order, the data might not be usable. 
